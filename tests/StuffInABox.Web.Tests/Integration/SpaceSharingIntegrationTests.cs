...
public class SpaceSharingIntegrationTests : IClassFixture<WebApplicationFactory<Program>>, IDisposable
{
    ...
    public async Task NonMember_CannotAccess_UntilInviteAccepted()
    {
        ...
        var sp = await owner.PostAsJsonAsync("/api/spaces", new { name = "Vinden", icon = "ti-home" });
        var spaceId = (await sp.Content.ReadFromJsonAsync<JsonElement>()).GetProperty("spaceId").GetString()!;
        var bx = await owner.PostAsJsonAsync("/api/boxes", new { spaceId, label = "Jul" });
        ...
        var spaces = await member.GetFromJsonAsync<JsonElement>("/api/spaces");
        ...
        var addItem = await member.PostAsJsonAsync($"/api/boxes/{boxNumber}/items",
        ...
        var deleteSpace = await member.DeleteAsync($"/api/spaces/{spaceId}");
    ...
    public async Task RevokedInvite_CannotBeRedeemed()
    {
        ...
        var sp = await owner.PostAsJsonAsync("/api/spaces", new { name = "Förråd", icon = "ti-box" });
        var spaceId = (await sp.Content.ReadFromJsonAsync<JsonElement>()).GetProperty("spaceId").GetString()!;

        var inviteResp = await owner.PostAsync($"/api/spaces/{spaceId}/invite", null);
        var token = (await inviteResp.Content.ReadFromJsonAsync<JsonElement>()).GetProperty("token").GetString();
        await owner.DeleteAsync($"/api/spaces/{spaceId}/invite");
